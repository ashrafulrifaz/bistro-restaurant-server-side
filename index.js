...
const uri = `mongodb+srv://${process.env.VITE_USER}:${process.env.VITE_PASSWORD}@cluster0.klq4o7m.mongodb.net/?retryWrites=true&w=majority`;
...
async function run() {
  try {
    ...
    app.get('/admin-status', verifyToken, verifyAdmin, async(req, res) => {
      ...
      res.send({usersCount, productsCount, ordersCount, revenueCount})
    })
    ...
    app.put('/menu/:id', verifyToken, verifyAdmin, async(req, res) => {
      ...
      const result = await menuCollection.updateOne(query, updateItem)
      res.send(result)
    })
